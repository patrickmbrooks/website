...
struct SendView: View {
    ...
    private func sendViaDocuSign() async {
        ...
        do {
            ...
            let id = try await client.sendForSignature(pdf: pdf, retainer: retainer, firm: settings.firm)
            retainer.docusignEnvelopeId = id
            retainer.status = .sentForSignature
            store.upsert(retainer)
            message = "Sent. DocuSign will email \(retainer.clientEmail) a signing link. Envelope \(id)."
        } catch {
    ...
    private func finishInPersonSignature(_ image: UIImage) {
        ...
        """
        let signed = PDFGenerator(firm: settings.firm, retainer: retainer, clientSignature: image, auditFooter: audit).makePDF()
